...
@st.fragment
def _plate_panel():
    """Runs independently.  st.rerun() here only reruns this fragment."""
    ...
    dirty = False       # becomes True when any selection button is pressed
    ...
    rtxt = c1.text_input("rng", label_visibility="collapsed",
                          placeholder="A1:A3   or   B1:D6   or   G10, H11",
                          key="range_in")
    if c2.button("Add", key="radd", type="primary", use_container_width=True):
    ...
    if c3.button("Remove", key="rrem", use_container_width=True):
        parsed = set(parse_well_range(rtxt))
        sel -= parsed
        dirty = True
    ...
    if dirty:
        ...
        st.rerun()          # only this fragment reruns — right panel untouched
    ...
    if st.session_state.selected_wells:
        ...
        st.info("No wells selected yet.  Use the row / column buttons or type a range above.")
...
def _assign_panel():
    ...
    if u1.button("↩ Undo last", disabled=not st.session_state.undo_stack,
        ...
        # no explicit st.rerun() — button click already triggers a full rerun
