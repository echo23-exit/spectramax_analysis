...
# ─────────────────────────────────────────────────────────────────────────────
# PLATE VISUALISATION  (Plotly — interactive)
# ─────────────────────────────────────────────────────────────────────────────
...
def tab_designer():
    ...
    with left:
        st.markdown("**Click wells to select · box-drag to select a region · click again to deselect**")
        ...
        # Sync Plotly selection → session state
        if event and hasattr(event, "selection") and event.selection:
            ...
            for pt in pts:
                ...
                if isinstance(cd, str) and re.match(r"^[A-H]\d{1,2}$", cd):
                    new_sel.append(cd)
            if new_sel != st.session_state.selected_wells:
                st.session_state.selected_wells = new_sel

        if st.session_state.selected_wells:
            sel_sorted = sorted(st.session_state.selected_wells, key=well_sort_key)
            st.info(f"Selected **{len(sel_sorted)}** well(s): {', '.join(sel_sorted)}")
        else:
    ...
    with right:
        ...
        if assign_clicked and can_assign:
            ...
            # Check if condition already exists — allow adding more wells
            ...
            st.session_state.selected_wells = []
            st.session_state.chart_ver += 1     # force fresh chart
            st.rerun()
        ...
        if u2.button("🗑 Clear all"):
            for k in ("assignments", "conditions_list", "selected_wells",
                ...
                st.session_state[k] = {} if k == "assignments" else ([] if k != "template_csv" else None)
            st.session_state.chart_ver += 1
            st.rerun()
...
1. **Click** individual wells on the plate, or **drag a box** to select a rectangular region.
2. Type a **condition name** and pick a **colour**.
3. Click **Assign wells** — they are coloured immediately and appear in the legend.
4. Typing the *same* condition name and assigning more wells **adds** to that condition.
